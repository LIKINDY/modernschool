...
$is_teacher_user = in_array(strtolower((string)$user_role), ['teacher', 'class teacher'], true);

if ($user_role === 'teacher' || strtolower($user_role) === 'class teacher') {
    $tRes = $conn->query("SELECT teaching_level, fullname FROM teachers WHERE id='" . (int)$user_id . "' LIMIT 1");
    $tData = $tRes ? $tRes->fetch_assoc() : null;
    $level = normalize_teacher_level($tData['teaching_level'] ?? '');
    if (!in_array($level, ['primary', 'primery', 'primari', 'nursery', 'nursary', 'nurssary'], true)) {
...
if (isset($_GET['load_list'])) {
    ...
    // Query to handle M1, M2 and Exam marks
    ...
    if (isset($_REQUEST['request_unlock']) && ($user_role === 'teacher' || strtolower($user_role) === 'class teacher')) {
        ...
        create_marks_edit_request($conn, $ctx, (int)$user_id, $teacherName, $reason);
        $request_status = 'Unlock request sent to admin.';
    }

    $lock_state = normalize_lock_state($conn, get_marks_lock_state($conn, $context_hash));
    $is_locked = is_marks_context_locked($lock_state);
    $completion_percent = (float)($lock_state['completion_percent'] ?? 0);

    $pendingRes = $conn->query("SELECT COUNT(*) AS total FROM marks_edit_requests WHERE context_hash='" . mysqli_real_escape_string($conn, $context_hash) . "' AND teacher_id='" . (int)$user_id . "' AND status='pending'");
    $pending = (int)(($pendingRes ? $pendingRes->fetch_assoc() : ['total' => 0])['total'] ?? 0);
    if ($pending > 0) {
        $request_status = 'You already have a pending unlock request.';
    }
...
<head>
    ...
    <style>
        ...
        .header-section { background: linear-gradient(135deg, #4361ee, #3f37c9); color: white; padding: 20px; border-radius: 15px; margin-bottom: 25px; }
...
<div class="content container-fluid py-4">
    <div class="row justify-content-center">
        <div class="col-lg-11">
            ...
            <div class="header-section d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center p-4 mb-4 shadow-sm" style="background: linear-gradient(135deg, #4361ee 0%, #3f37c9 100%); color: white; border-radius: 16px;">
    ...
    <div class="d-flex flex-wrap gap-2">
        <a href="primary_emport_excel.php" class="btn btn-light btn-sm fw-bold px-3 py-2 rounded-3 text-success shadow-sm d-flex align-items-center transition-all">
            <i class="fas fa-file-upload me-2"></i> Import Result (Excel)
        </a>
        
        <a href="scan_result.php?level=Primary&class=<?= @$_GET['class'] ?>&stream=<?= @$_GET['stream'] ?>&subject=<?= @$_GET['subject'] ?>&year=<?= @$_GET['year'] ?>&exam_type=<?= @$_GET['exam_type'] ?>" class="btn btn-warning btn-sm fw-bold px-3 py-2 rounded-3 shadow-sm d-flex align-items-center transition-all">
            <i class="fas fa-camera me-2"></i> Scan (Camera / Excel)
        </a>

        <a href="primary_results.php" class="btn btn-white btn-sm fw-bold px-3 py-2 rounded-3 shadow-sm d-flex align-items-center text-primary bg-white opacity-90" style="border: 1px solid rgba(255,255,255,0.2);">
            ...
            <?php if ($is_locked && $is_teacher_user): ?>
                <div class="alert alert-warning border-0 shadow-sm d-flex flex-wrap justify-content-between align-items-center gap-3">
                    ...
                    <?php if ($user_role === 'teacher' || strtolower($user_role) === 'class teacher'): ?>
            ...
            <div class="table-container shadow-sm">
                <form action="save_primary_marks.php" method="POST">
                    ...
                    <div class="weight-box d-flex align-items-center">
                        ...
                        <?php else: ?>
                            <div class="me-4">
                                ...
                                <input type="number" name="m_weight" id="m_weight" class="form-control form-control-sm w-75 fw-bold" value="40" oninput="updateExamWeight(this.value); updateAllGrades();">
                            </div>
                            <div>
                    ...
                    <table class="table table-hover align-middle <?= ($is_locked && $is_teacher_user) ? 'opacity-75' : '' ?>">
                                ...
                                <tr class="student-row">
                                    ...
                                    <td class="text-center">
                                        <input type="number" name="m_marks[]" class="form-control mark-input m-mark mx-auto" step="0.01" min="0" max="100" 
                                               value="<?php echo $std['monthly_mark']; ?>" oninput="calculateRowGrade(this); queueAutoSave(this)" <?= ($is_locked && $is_teacher_user) ? 'disabled' : '' ?>>
                                    </td>
                                    <?php elseif($isSpecial): ?>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">
                                        <input type="number" name="m2_marks[]" class="form-control mark-input m2-mark mx-auto" step="0.01" min="0" max="20" 
                                               value="<?php echo $std['m2_mark']; ?>" oninput="calculateRowGrade(this); queueAutoSave(this)" <?= ($is_locked && $is_teacher_user) ? 'disabled' : '' ?>>
                                    </td>
                                    <?php endif; ?>

                                    <td class="text-center">
                                        <input type="number" name="e_marks[]" class="form-control mark-input e-mark mx-auto" step="0.01" min="0" max="100" 
                                               value="<?php echo $std['exam_mark']; ?>" oninput="calculateRowGrade(this); queueAutoSave(this)" <?= ($is_locked && $is_teacher_user) ? 'disabled' : '' ?>>
                                    </td>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mt-4">
                        <small id="autosaveStatus" class="text-muted fw-bold">
                            <i class="fas fa-bolt me-1 text-primary"></i> Auto-save active
                        </small>
                        <button type="submit" class="btn btn-success btn-lg px-5 fw-bold shadow" <?= ($is_locked && $is_teacher_user) ? 'disabled' : '' ?>>
...
function calculateRowGrade(inputElement) {
    ...
    if (examType === 'special') {
        ...
        // Final score calculation based on weight
        total = (mVal) + (eVal * (ew/100));
    } 
    else {
        // Terminal or Annual: 100% Exam
    ...
    // Tanzania Primary Grading System
    let grade = '';
...
function queueAutoSave(inputElement) {
    const row = inputElement.closest('.student-row');
    if (!row) return;
    ...
    setAutosaveStatus('<i class="fas fa-spinner fa-spin me-1"></i> Saving...', 'warning');
...
function autoSaveRow(row) {
    ...
    const mWeightEl = document.getElementById('m_weight');
    const eWeightEl = document.getElementById('e_weight_hidden');

    const payload = {
        student_id: studentId,
        subject_id: "<?php echo @$_GET['subject']; ?>",
        class_name: "<?php echo @$_GET['class']; ?>",
        stream: "<?php echo @$_GET['stream']; ?>",
        academic_year: "<?php echo @$_GET['year']; ?>",
        exam_type: "<?php echo @$_GET['exam_type']; ?>",
        m_mark: row.querySelector('.m-mark, .m1-mark')?.value || '',
        m2_mark: row.querySelector('.m2-mark')?.value || '',
        e_mark: row.querySelector('.e-mark')?.value || '',
        m_weight: mWeightEl ? mWeightEl.value : 40,
        e_weight: eWeightEl ? eWeightEl.value : 60
    };

    fetch('save_primary_mark_auto.php', {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            setAutosaveStatus('<i class="fas fa-check-circle me-1"></i> Auto-saved at ' + (data.saved_at || 'now'), 'success');
        } else {
            setAutosaveStatus('<i class="fas fa-triangle-exclamation me-1"></i> Auto-save failed', 'danger');
        }
    })
    .catch(() => {
        setAutosaveStatus('<i class="fas fa-wifi me-1"></i> Network error on auto-save', 'danger');
    });
}
