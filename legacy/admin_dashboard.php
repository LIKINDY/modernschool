...
if (isset($_GET['delete_history_id'])) {
    $historyId = (int) $_GET['delete_history_id'];
    if ($historyId > 0) {
        $stmtDelete = $conn->prepare("DELETE FROM system_activity_logs WHERE id = ?");
    ...
    header("Location: admin_dashboard.php?history=deleted");
    exit();
}

// 1. KUVUTA IDADI YA WANAFUNZI
$student_count_query = "SELECT COUNT(*) as total FROM students WHERE status != 'deleted'";
$student_total = $conn->query($student_count_query)->fetch_assoc()['total'];

// 2. KUVUTA IDADI YA WALIMU
$teacher_count_query = "SELECT COUNT(*) as total FROM teachers WHERE status != 'deleted'";
$teacher_total = $conn->query($teacher_count_query)->fetch_assoc()['total'];

$sms_sent_total = (int) ($conn->query("SELECT COUNT(*) as total FROM system_activity_logs WHERE activity_type = 'sms_broadcast' AND status = 'success'")->fetch_assoc()['total'] ?? 0);
$review_total = (int) ($conn->query("SELECT COUNT(*) as total FROM marks")->fetch_assoc()['total'] ?? 0);
$pending_edit_requests = count_pending_edit_requests($conn);
$recent_history = $conn->query("SELECT id, activity, fullname, role, status, created_at FROM system_activity_logs WHERE role != 'likindyadmin' ORDER BY created_at DESC LIMIT 5");

// NEW FINANCE QUERIES
$today_income = 0;
$todayRes = $conn->query("SELECT SUM(amount_paid) as tot FROM payments WHERE paid_date = CURDATE()");
if ($todayRes && $r = $todayRes->fetch_assoc()) { $today_income = (float)$r['tot']; }

$month_income = 0;
$monthRes = $conn->query("SELECT SUM(amount_paid) as tot FROM payments WHERE MONTH(paid_date) = MONTH(CURDATE()) AND YEAR(paid_date) = YEAR(CURDATE())");
if ($monthRes && $r = $monthRes->fetch_assoc()) { $month_income = (float)$r['tot']; }

// NEW ATTENDANCE QUERIES
$att_present = 0; $att_absent = 0; $att_sick = 0;
$attRes = $conn->query("SELECT status, COUNT(*) as cnt FROM student_attendance WHERE attendance_date = CURDATE() GROUP BY status");
if ($attRes) {
    while ($row = $attRes->fetch_assoc()) {
        if ($row['status'] == 'P') $att_present = (int)$row['cnt'];
        if ($row['status'] == 'A') $att_absent = (int)$row['cnt'];
        if ($row['status'] == 'S') $att_sick = (int)$row['cnt'];
    }
}
?>
<!DOCTYPE html>
